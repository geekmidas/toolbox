...
describe('CacheAuditStorage', () => {
  let cache: InMemoryCache;
  let storage: CacheAuditStorage;

  beforeEach(() => {
    cache = new InMemoryCache();
    storage = new CacheAuditStorage({ cache });
  });

  describe('write', () => {
    it('should write records to cache', async () => {
    ...
    it('should serialize and deserialize dates correctly', async () => {
      ...
      expect(records[0].timestamp.toISOString()).toBe(timestamp.toISOString());
    });
  });
  ...
  describe('configuration', () => {
    it('should use custom prefix', async () => {
      const customStorage = new CacheAuditStorage({
        cache,
        prefix: 'my-audit',
      });

      await customStorage.write([
        {
          id: 'audit-1',
          type: 'test',
          operation: 'CUSTOM',
      ...
      // Check that it uses the custom prefix
      const indexValue = await cache.get<string[]>('my-audit:__index__');
      expect(indexValue).toContain('audit-1');

      const recordValue = await cache.get('my-audit:audit-1');
    ...
    it('should use custom TTL', async () => {
      ...
      const records = await customStorage.getRecords();
  ...
  describe('type safety', () => {
    it('should support generic audit action types', async () => {
      ...
      const typedStorage = new CacheAuditStorage<AppAuditAction>({ cache });
