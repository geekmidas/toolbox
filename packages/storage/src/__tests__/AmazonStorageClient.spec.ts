...
/**
 * Needs MinIO on localhost:9000 with the credentials below and a `geekmidas`
 * bucket — `docker compose up` at the repo root. The suite self-skips when that
 * is not available, so `pnpm test` stays green on a machine without it and
 * still exercises the real backend when there is one.
 */
const MINIO = {
	bucket: 'geekmidas',
	region: 'us-east-1',
...
const minioAvailable = await AmazonStorageClient.create(MINIO)
	.getVersions('probe')
	.then(() => true)
	.catch(() => false);

describe.skipIf(!minioAvailable)(
	'AmazonStorageClient Integration Tests',
	() => {
		...
		describe('custom expiration times', () => {
			it('should respect custom expiration for download URLs', async () => {
				...
				const shortExpirationUrl = await client.getDownloadURL(
					{ path: key },
					1,
				); // 1 second
